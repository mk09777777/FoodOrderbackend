...
router.post('/spice-villa', async (req, res) => {
  try {
    ...
    if (existingRestaurant) {
      ...
      res.json({ message: 'Spice Villa updated successfully!', restaurant: existingRestaurant._id });
    } else {
      const spiceVilla = new Restaurant(spiceVillaData);
      await spiceVilla.save();
      res.json({ message: 'Spice Villa created successfully!', restaurant: spiceVilla._id });
    }
  } catch (error) {
    res.status(500).json({ message: 'Error seeding Spice Villa', error: error.message });
  }
});
